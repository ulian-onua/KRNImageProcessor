...
open class KRNImageCropper {
    ...
    open func cropFromRightToLeft(image: UIImage, width: CGFloat) -> UIImage? {
        return crop(image: image, rect: CGRect(x: image.size.width - width,
                                               y: 0,
                                               width: width,
                                               height: image.size.height))
    }
    
    // MARK: - Center croppers
    ...
    open func cropRectangleFromCenter(image: UIImage, width: CGFloat, height: CGFloat) -> UIImage? {
        ...
        return crop(image: image, rect: CGRect(x: startX,
                                               y: startY,
                                               width: width,
                                               height: height))
    }
    ...
    // MARK: - All sides cropper
    
    open func cropSides(image: UIImage,
                        top: CGFloat = 0.0,
                        right: CGFloat = 0.0,
                        left: CGFloat = 0.0,
                        bottom: CGFloat = 0.0) -> UIImage? {
